...
class DataValidation:
    ...
    def validate_no_of_columns(self,dataframe:pd.DataFrame)-> bool:
        ...
        ## Bool returen type of the function
        try:
            no_of_columns=len(self._schema_config["columns"])
            logging.info(f"Required number of columns are {no_of_columns}")
            logging.info(f"Columns in dataframe are {len(dataframe.columns)}")
        ...
        try:
            ...
            for column in schema_numerical_columns:
                if column not in dataframe.columns:
                    logging.error(f"{column} is missing.")
                    return False
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            ## read the data from train and test set 
            ...
            if not status:
                print("Train data does not contains all columns\n")
            status=self.validate_no_of_columns(test_dataframe)
            if not status:
                print("Test data does not contains all columns\n")
            ## Validate numerical columns 
            status=self.validate_numerical_columns(train_dataframe)
            if not status:
                print("Train data does not contains all numerical columns\n")
            status=self.validate_numerical_columns(test_dataframe)
            if not status:
                print("Test data does not contains all numerical columns\n")
            ## lets check dataset drift
